...
Both model IDs exist in the pinned RAGFlow OpenAI catalog. Credentials must remain
in environment/secret storage and must never enter manifests or command arguments.
...
4. `ragflow_generated_three_query`: original plus two alternatives generated by
   the same `gpt-5.4-mini` prompt used by RAGZ, adapter-side RRF.
